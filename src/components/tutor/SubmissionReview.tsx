  ...
  return (
    <>
      <section className="mt-6 rounded-lg border border-line bg-paper p-4">
        ...
        <p className="mt-1 whitespace-pre-wrap text-sm text-slate">{markingGuide}</p>
        <p className="mt-2 text-xs text-slate">Only you can see this.</p>
      </section>
      ...
      {shown.length === 0 ? (
        ...
        <ul className="mt-4 space-y-2">
          {shown.map((row) => (
            <li key={row.submissionId} className="rounded-lg border border-line bg-chalk">
              <button
                type="button"
                onClick={() =>
              ...
              >
                ...
                <span className="shrink-0 text-right text-sm">
                  {row.status === "finalised" ? (
                    ...
                    <span className="text-slate">Not marked</span>
                  )}
                </span>
              </button>
  ...
  return (
    <div className="border-t border-line p-4">
      <section>
        ...
        {detail?.content ? (
          <p className="mt-2 max-h-64 overflow-y-auto whitespace-pre-wrap rounded-lg border border-line bg-paper p-3 text-sm">
            {detail.content}
          </p>
        ) : (
          <p className="mt-2 text-sm text-slate">They sent files only.</p>
        )}
      </section>
        ...
        <ul className="mt-3 space-y-2">
          {detail.attachments.map((file) => (
            <li key={file.href}>
              <a
                href={file.href}
                className="flex items-center justify-between gap-3 rounded-lg border border-line bg-paper px-3 py-2 text-sm hover:border-marker"
              >
      ...
      {row.status === "ai_grading_failed" && (
        <p className="mt-5 rounded-lg border border-line bg-paper p-3 text-sm">
      ...
      */}
      {(SWEEPABLE_STATUSES as readonly string[]).includes(row.status) && (
        <div className="mt-5 rounded-lg border border-line bg-paper p-3 text-sm">
          {row.gradingAttempts >= GRADING_ATTEMPT_CAP ? (
            ...
            <>
              ...
              <button
                ...
                disabled={retrying}
                onClick={() => void retry()}
                className="mt-2 font-medium text-marker disabled:opacity-60"
              >
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-line bg-paper px-3 py-2 disabled:opacity-60"
          />
        </label>
        ...
        {row.aiScore !== null && !released && (
          <button
            ...
            className="mt-6 h-11 rounded-lg border border-line bg-paper px-4 font-medium text-marker"
          >
            Use AI mark
          </button>
        )}
      ...
      {queued && (
        <p role="status" className="mt-4 rounded-lg border border-line bg-paper p-3 text-sm">
      ...
      {released ? (
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <button
            ...
            className="rounded-lg border border-line bg-paper px-4 py-2 font-medium disabled:opacity-50"
          >
